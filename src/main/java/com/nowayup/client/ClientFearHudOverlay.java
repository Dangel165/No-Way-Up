...
public final class ClientFearHudOverlay {
    ...
    public static void onRenderGui(RenderGuiOverlayEvent.Post event) {
        ...
        int textY = y;
        graphics.drawString(font, "No Way Up", x, textY, 0xD8D8D8, true);
        textY += LINE_HEIGHT + 1;
        graphics.drawString(font, "Fear: " + ClientFearHudState.fear(), x, textY, fearColor(ClientFearHudState.fear()), true);
        textY += LINE_HEIGHT;
        graphics.drawString(font, "Phase: " + ClientFearHudState.phase(), x, textY, 0xB8B8B8, true);
        textY += LINE_HEIGHT;
        graphics.drawString(font, "Layer: " + (ClientFearHudState.mirror() ? "Mirror" : "Mine"), x, textY, 0xA7C7D9, true);
        textY += LINE_HEIGHT;
        graphics.drawString(font, "False exits: " + ClientFearHudState.fakeExits(), x, textY, 0xB8B8B8, true);
        if (!ClientFearHudState.ending().isEmpty()) {
            textY += LINE_HEIGHT;
            graphics.drawString(font, ClientFearHudState.ending(), x, textY, 0xFFE08A, true);
        }
    }
